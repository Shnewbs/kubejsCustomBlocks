...
StartupEvents.registry("block", (event) => {
    ...
    event.create(`${TFCStoneTypes}.cobble.block.2`) // Create a new block
.displayName(`${displayMaterial} Cobble Block 2`) // Set a custom name
.soundType("stone") // Set a material (affects the sounds and some properties)
.hardness(1.0) // Set hardness (affects mining time)
.resistance(1.0) // Set resistance (to explosions, etc)
.textureAll(`kubejs:block/tfc/block/rock/cobble2/${TFCStoneTypes}`)
